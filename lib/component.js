...
const svl = ({ specRenderTransform = x => x }) => {
  ...
  const createComponentFactory = (oldSpec = {}) => (...args) => {
    ...
    const finalSpec = mergeSpec(oldSpec, specPassed)
  ...
  const component = spec => {
    ...
    const update = props => {
      ...
      eachKey(
        (name, value) => {
          if (updateListeners[name]) {
            updateListeners[name].forEach(update => {
              updates.push({ path: update.path, value: props[name] })
            })
          }
        },
        props || {}
      ...
      newSpec.children = (newSpec.children || []).map(child => child.isComponent ? child.update(props) : child)
      return updates ? component(newSpec) : self
    }
    self.update = update

    const getChildHtml = () => spec.children.map(child =>
